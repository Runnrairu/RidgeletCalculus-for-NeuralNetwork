...
def inference(condition_placeholder):
  ...
  with tf.name_scope("output") as scope:  
    output = Z*(tf.matmul(hidden1_output, output_weight) + output_bias)
...
def training(loss):
  with tf.name_scope("training") as scope:
    train_step = tf.train.AdamOptimizer(0.01).minimize(loss,var_list=[hidden1_weight,hidden1_bias,output_weight,output_bias])
  return train_step
